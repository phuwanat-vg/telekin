...
directly: `sudo apt install ./telekin-host_1.0.11-1_arm64.deb`.

### Computer

| Platform | Install | Upgrade |
|---|---|---|
| Windows 10/11 | `telekin-1.0.11-windows-x64-setup.exe` — per-user, no admin needed | run the newer setup; it replaces in place |
| Ubuntu | same repository as above, then `sudo apt install telekin` | `sudo apt upgrade` |
| macOS 11+ | `telekin-1.0.11-macos.dmg`, drag to Applications. Unsigned: right-click → Open the first time | drag the new one over |
